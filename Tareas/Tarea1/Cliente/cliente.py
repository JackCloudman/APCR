...
class Ui(QtWidgets.QMainWindow):
    ...
    def upload(self):
        ...
        if type(self.send) == str:
            ...
            message = json.dumps({"tipo":"folder","nombres":[name],"numero":1,"sizes":[self.getSize("temp/%s.zip"%name)]})
            ...
            message = json.dumps({"tipo":"list","nombres":[basename(x) for x in self.send],"numero":len(self.send),"sizes":[self.getSize(x) for x in self.send]})
            self.s.sendall(message.encode())
            time.sleep(0.05)
            for f in self.send:
                self.sendFile(f)
